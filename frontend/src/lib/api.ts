...
// Points to /api which Vite proxies to FastAPI on port 8000
const API_BASE = '/api';
...
async function request<T = any>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    catch { errBody = { error: { message: res.statusText, code: 'HTTP_ERROR' } }; }
    const detail = errBody?.detail || errBody?.error || {};
    const err = new Error(detail?.message || errBody?.message || 'An error occurred');
    (err as any).code = detail?.code || 'UNKNOWN_ERROR';
    (err as any).status = res.status;
    (err as any).retryable = detail?.retryable;
    throw err;
  }
...
export const api = {
  auth: {
    ...
    async logout(): Promise<void> {
      ...
      finally { setStoredToken(null); }
    },
    async getMe(): Promise<{ user: User }> { return request('/auth/me'); },
    async updateProfile(name: string): Promise<{ user: User }> {
      return request('/profile', { method: 'PATCH', body: JSON.stringify({ name }) });
    },
  },

  datasets: {
    async list(): Promise<{ datasets: Dataset[] }> { return request('/datasets'); },
  ...
  uploads: {
    ...
    async importDataset(uploadId: string, datasetName?: string, tableName?: string): Promise<{ success: boolean; message: string; dataset: any }> {
  ...
  analyses: {
    async create(datasetId: string, question: string): Promise<{ analysisId: string; status: string }> {
      return request('/analyses', { method: 'POST', body: JSON.stringify({ datasetId, question }) });
    },
  ...
  previews: {
    async confirm(previewId: string, digest?: string, idempotencyKey?: string): Promise<{ success: boolean; message: string; result: ExecutionResult }> {
  ...
  executions: {
    ...
    downloadCsvUrl(executionId: string, filterCol?: string, filterVal?: string): string {
      let url = `${API_BASE}/executions/${executionId}/export.csv`;
      if (filterCol && filterVal) url += `?filterCol=${encodeURIComponent(filterCol)}&filterVal=${encodeURIComponent(filterVal)}`;
      return url;
    },
  },
  ...
  health: {
    async check(): Promise<any> { return request('/health'); },
  },

  admin: {
    async listOrganizations(): Promise<{ organizations: any[] }> { return request('/admin/organizations'); },
    async registerOrganization(data: any): Promise<any> {
      return request('/admin/organizations', { method: 'POST', body: JSON.stringify(data) });
    },
    async joinOrganization(data: any): Promise<any> {
      return request('/admin/organizations/join', { method: 'POST', body: JSON.stringify(data) });
    },
    async getOrganizationDetails(orgId: string): Promise<any> { return request(`/admin/organizations/${orgId}`); },
    async updateOrganization(orgId: string, data: any): Promise<any> {
      return request(`/admin/organizations/${orgId}`, { method: 'PATCH', body: JSON.stringify(data) });
    },
    async getAuditLogs(orgId: string, filters?: any): Promise<any> {
    ...
    async listUsers(): Promise<{ users: any[] }> { return request('/admin/users'); },
    async updateUserPermissions(userId: string, permissionLevel: string): Promise<any> {
      return request(`/admin/users/${userId}/permissions`, { method: 'PATCH', body: JSON.stringify({ permissionLevel }) });
    },
    async deleteUser(userId: string): Promise<any> { return request(`/admin/users/${userId}`, { method: 'DELETE' }); },
    async updateMemberStatus(membershipId: string, status: string, rejectionReason?: string): Promise<any> {
      return request(`/admin/memberships/${membershipId}/status`, { method: 'POST', body: JSON.stringify({ status, rejectionReason }) });
    },
    async updateMemberPermissions(membershipId: string, databaseId: string, permissions: any): Promise<any> {
      return request(`/admin/memberships/${membershipId}/permissions/${databaseId}`, { method: 'PUT', body: JSON.stringify(permissions) });
    },
    async getInvite(token: string): Promise<any> { return request(`/invitations/${token}`); },
    async acceptInvite(token: string, name: string, password: string): Promise<any> {
      return request(`/invitations/${token}/accept`, { method: 'POST', body: JSON.stringify({ name, password }) });
    },
  },

  data: {
    async getTableRecords(datasetId: string, tableName: string, params?: any): Promise<any> {
      const query = new URLSearchParams(params || {});
      return request(`/datasets/${datasetId}/tables/${tableName}/records?${query.toString()}`);
    },
    async previewMutation(datasetId: string, tableName: string, body: any): Promise<any> {
      return request(`/datasets/${datasetId}/tables/${tableName}/records/preview`, { method: 'POST', body: JSON.stringify(body) });
    },
    async confirmOperation(previewId: string, digest: string, idempotencyKey?: string): Promise<any> {
      return request(`/operations/${previewId}/confirm`, { method: 'POST', body: JSON.stringify({ digest, idempotencyKey }) });
    },
    async appendCsv(datasetId: string, tableName: string, csvText: string): Promise<any> {
      return request(`/datasets/${datasetId}/tables/${tableName}/append-csv`, { method: 'POST', body: JSON.stringify({ csvText }) });
    },
    exportCsvUrl(datasetId: string, tableName: string): string {
      return `${API_BASE}/datasets/${datasetId}/tables/${tableName}/export.csv`;
    },
    async createDatabase(name: string, description?: string): Promise<any> {
      return request('/datasets', { method: 'POST', body: JSON.stringify({ name, description }) });
    },
    async deleteDatabase(datasetId: string, confirmedName: string): Promise<any> {
      return request(`/datasets/${datasetId}/delete`, { method: 'POST', body: JSON.stringify({ confirmedName }) });
    },
    async createTable(datasetId: string, tableName: string, columns: any[]): Promise<any> {
      return request(`/datasets/${datasetId}/tables`, { method: 'POST', body: JSON.stringify({ tableName, columns }) });
    },
    async dropTable(datasetId: string, tableName: string, confirmedName: string): Promise<any> {
      return request(`/datasets/${datasetId}/tables/${tableName}/drop`, { method: 'POST', body: JSON.stringify({ confirmedName }) });
    },
  },
};
